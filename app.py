...
Analyze only the information provided by the user. Never invent facts. Separate explicit facts from AI inference.
Reason about delivery timing, remaining runway, known activity duration, sequencing, critical-path pressure, contingency, and dependency impact.
Do not claim exact timing unless supported by the input.

IMPORTANT GUARDRAILS:
- Do not create a risk, blocker, or dependency merely because something is unknown.
- If the user explicitly says there are no known external blockers, do not create an unknown-blockers risk.
- Do not contradict explicit facts. If evidence is mixed, describe uncertainty.

Return valid JSON with exactly these keys:
executive_summary, program_health, delivery_intelligence, risks, blockers, dependencies, recommended_next_actions, leadership_attention, facts, inferences.
program_health: status (Green, Amber, or Red), reason.
delivery_intelligence: timeline_assessment, schedule_pressure, contingency_assessment.
risk objects: risk, impact, mitigation.
dependency objects: dependency, why_it_matters.
leadership_attention: decision_or_escalation, why_now.
facts and inferences: concise arrays.
Actions: concise, ordered, specific."""
...
DEMO_RESULT = {
    ...
    "risks": [{"risk": "Security review has not started", "impact": "Could delay go-live if it cannot complete within the remaining runway.", "mitigation": "Start immediately and confirm prerequisites and lead time."}, {"risk": "Vendor API issue remains unresolved", "impact": "May consume up to one week and affect downstream testing.", "mitigation": "Obtain a committed resolution date and confirm whether it blocks testing."}],
...
def _zero_state(message: str) -> None:
    st.markdown(f"<div class='zero-state'>✓ {message}</div>", unsafe_allow_html=True)
...
def render_snapshot(result: dict[str, Any]) -> None:
    ...
    cards = [("Program Health", status, _status_class(status)), ("Schedule Pressure", pressure, _status_class(pressure)), ("Contingency", contingency, _contingency_class(contingency)), ("Blockers", str(blockers), "red" if blockers else "green")]
...
def render_result(result: dict[str, Any]) -> None:
    ...
    .zero-state {padding:8px 10px;margin:6px 0;color:#6fcf72;background:rgba(92,184,92,.06);border-left:3px solid #5cb85c;border-radius:4px;}
    ...
    with col1:
        ...
        if result["risks"]:
            ...
            _zero_state("No active risks identified")
        st.subheader("🚧 Blockers")
        if result["blockers"]:
            for item in result["blockers"]:
                st.write(f"- {item}")
        else:
            _zero_state("No active blockers")
        st.subheader("📌 Facts")
        facts = result.get("facts", [])
        if facts:
            for item in facts:
            ...
            st.info("No explicit facts were extracted from the update.")
    with col2:
        ...
        if result["dependencies"]:
            ...
            _zero_state("No external dependencies identified")
